...
>유선(*트위스트 페어 케이블, 광케이블 등*)과 
>무선(*라디오파, 마이크로파, 적외선 등*)이 있다
...
>이처럼 허브는 스스로 판단하지 않고, 전기 신호를 모든 포트로 보내서 `더미 허브(dummy hub)`라는 이름으로 불리기도 한다<br />
><span style="background-color:yellow">더미 허브 문제점의 대책으로 나온게 **스위치(switch)**</span>라고 하는 네트워크 장비다
